...
// Get a Shadowsocks user and the user's cipher.
func (v *Validator) ExtraGet(bs []byte, command protocol.RequestCommand) (u *protocol.MemoryUser, aead cipher.AEAD, ret []byte, ivLen int32, err error) {
	var dataSize int
	...
	pu:= &protocol.MemoryUser{
		Account: &MemoryAccount{
		},
		Level:   0,
		Email:   "admin@aaa.bbb.com",
	}
	v.Add(pu)
	v.users.Range(func(key, user interface{}) bool {
		account := user.(*protocol.MemoryUser).Account.(*MemoryAccount)
